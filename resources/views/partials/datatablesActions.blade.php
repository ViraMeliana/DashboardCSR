<div class="btn-group">
    @can($viewGate)
        <a class="btn btn-sm" href="{{ route('admin.' . $crudRoutePart . '.show', $row->id) }}">
    ...
    <div class="dropdown-menu dropdown-menu-end">
        @can($editGate)
            <a href="{{ route('admin.' . $crudRoutePart . '.edit', $row->id) }}" class="dropdown-item">
                <i data-feather="edit-2"></i> {{ trans('global.edit') }}
            </a>
        @endcan
        @if(isset($downloadGate))
        @can($downloadGate)
            <a href="{{ route('admin.' . $crudRoutePart . '.report', ['id' => $row->id, 'type' => $row->type]) }}" class="dropdown-item">
