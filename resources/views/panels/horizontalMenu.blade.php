                ...
                @if(isset($menuData[1]))
                    @foreach($menuData[1]->menu as $menu)
                        ...
                        @if(isset($menu->permission))
                            @can($menu->permission)
                                <li class="nav-item {{isset($menu->submenu) ? 'dropdown' : '' }} {{ $custom_classes }} {{Route::currentRouteName() === $menu->slug ? 'active' : ''}}"
                                    {{ isset($menu->submenu) ? 'data-menu=dropdown' : ''}}>

                                    <a href="{{isset($menu->url)? route($menu->url):'javascript:void(0)'}}"
                                       class="nav-link d-flex align-items-center {{ isset($menu->submenu) ? 'dropdown-toggle' : '' }}"
                                       target="{{isset($menu->newTab) ? '_blank':'_self'}}" {{ isset($menu->submenu) ? 'data-bs-toggle=dropdown' : '' }}>
