...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.tjsls.update", [$tjsl->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tpbs') ? 'is-invalid' : '' }}" name="tpbs[]" id="tpbs" multiple>
                    @foreach($tpbs as $id => $tpb)
                        <option value="{{ $id }}" {{ (in_array($id, old('tpbs', [])) || $tjsl->tpbs->contains($id)) ? 'selected' : '' }}>{{ $tpb }}</option>
                    @endforeach
                </select>
                @if($errors->has('tpbs'))
