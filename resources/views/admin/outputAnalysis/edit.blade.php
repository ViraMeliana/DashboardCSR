        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">
                        {{ trans('global.edit') }} {{ trans('cruds.outputAnalysi.title_singular') }}
                    </h4>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route("admin.output-analysis.update", [$outputAnalysi->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="col-xl-12 col-md-12 col-12">
                                <div class="mb-1">
                                    ...
                                    <input class="form-control {{ $errors->has('output') ? 'is-invalid' : '' }}" type="text" name="output" id="output" value="{{ old('output', $outputAnalysi->output) }}" required>
                                ...
                                <div class="mb-1">
                                    ...
                                    <select class="form-control select2 {{ $errors->has('analysis_process') ? 'is-invalid' : '' }}" name="analysis_process_id" id="analysis_process_id">
                                        @foreach($analysis_processes as $id => $entry)
                                            <option value="{{ $id }}" {{ (old('analysis_process_id') ? old('analysis_process_id') : $outputAnalysi->analysis_process->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                        @endforeach
                                    </select>
